...
export async function setRdoStatus(formData: FormData) {
  ...
  if (!rdoId || !["draft", "submitted", "review", "approved"].includes(status)) {
...
export async function updateMemberRole(formData: FormData) {
  ...
  const orgId = asString(formData.get("organization_id"));
  const role = asString(formData.get("role"));
  if (!profileId || !orgId || !role) throw new Error("dados obrigatórios faltando");
  if (!["owner", "admin", "engineer", "viewer"].includes(role)) throw new Error("role inválido");
...
export async function removeMember(formData: FormData) {
  ...
  const profileId = asString(formData.get("profile_id"));
  const orgId = asString(formData.get("organization_id"));
  if (!profileId || !orgId) throw new Error("dados obrigatórios");
  const { error } = await supabase
    .from("organization_members")
    .delete()
    .eq("profile_id", profileId)
...
export async function updateActivity(formData: FormData) {
  ...
  const progress = asNum(formData.get("progress_pct"));
  const notes = asString(formData.get("notes")) || null;
  const { error } = await admin.from("report_activities").update({
    description: description || undefined,
    progress_pct: progress,
    notes,
  } as never).eq("id", id);
  if (error) throw new Error(error.message);
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
}
...
export async function deleteActivity(formData: FormData) {
  ...
  const siteId = asString(formData.get("siteId"));
  if (!id) throw new Error("id obrigatório");
  await admin.from("report_activities").delete().eq("id", id);
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
}
...
export async function updatePhotoCaption(formData: FormData) {
  ...
  const siteId = asString(formData.get("siteId"));
  if (!id) throw new Error("id obrigatório");
  await admin.from("media").update({ caption } as never).eq("id", id);
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
  if (siteId) revalidatePath(`/obras/${siteId}/fotos`);
}
...
export async function addMaterial(formData: FormData) {
  ...
  const siteId = asString(formData.get("siteId"));
  const name = asString(formData.get("name"));
  const unit = asString(formData.get("unit")) || null;
  const quantity = asNum(formData.get("quantity"));
  const notes = asString(formData.get("notes")) || null;
  if (!rdoId || !name) throw new Error("dados obrigatórios");
  await supabase.from("report_materials").insert({
    daily_report_id: rdoId, name, unit, quantity, notes,
  } as never);
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
}
...
export async function deleteMaterial(formData: FormData) {
  ...
  const siteId = asString(formData.get("siteId"));
  if (!id) throw new Error("id obrigatório");
  await supabase.from("report_materials").delete().eq("id", id);
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
}

/* ───────── Templates ───────── */

export async function saveRdoTemplate(formData: FormData) {
  const { supabase, user } = await requireUser();
  const name = asString(formData.get("name"));
  const wf = asString(formData.get("workforce_json")) || "[]";
  const eq = asString(formData.get("equipment_json")) || "[]";
  const ac = asString(formData.get("activities_json")) || "[]";
  if (!name) throw new Error("nome do template obrigatório");
  const { data: profileR } = await supabase.from("profiles").select("default_org_id").eq("id", user.id).maybeSingle();
  const orgId = (profileR as { default_org_id?: string } | null)?.default_org_id;
  if (!orgId) throw new Error("Sem organização");
  await supabase.from("rdo_templates").insert({
    organization_id: orgId, name,
    workforce: JSON.parse(wf),
    equipment: JSON.parse(eq),
    activities: JSON.parse(ac),
    created_by: user.id,
  } as never);
}

export async function deleteRdoTemplate(formData: FormData) {
  const { supabase } = await requireUser();
  const id = asString(formData.get("id"));
  if (!id) throw new Error("id obrigatório");
  await supabase.from("rdo_templates").delete().eq("id", id);
}
...
export async function saveSignature(formData: FormData) {
  ...
  const rdoId = asString(formData.get("rdoId"));
  const siteId = asString(formData.get("siteId"));
  const dataUrl = asString(formData.get("data_url"));
  const signerName = asString(formData.get("signer_name")) || null;
  if (!rdoId || !dataUrl) throw new Error("dados obrigatórios");
  await supabase.from("daily_reports").update({
    signature_data_url: dataUrl,
    signature_signer_name: signerName,
    signature_signed_at: new Date().toISOString(),
  } as never).eq("id", rdoId);
  void user;
  if (siteId && rdoId) revalidatePath(`/obras/${siteId}/rdos/${rdoId}`);
}
