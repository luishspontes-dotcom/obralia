...
export default async function RdoDetailPage({
  ...
  const [actsR, wfR, eqR, photosR, videosR, filesR, commentsR, matsR, prevR, nextR, orgR] = await Promise.all([
    ...
    supabase.from("media").select("id, storage_path, thumbnail_path, caption, wbs_item_id").eq("daily_report_id", rdoId).in("external_provider", MEDIA_SOURCE_PROVIDERS).eq("kind", "photo").limit(60),
    supabase.from("media").select("id, storage_path, thumbnail_path, caption").eq("daily_report_id", rdoId).in("external_provider", MEDIA_SOURCE_PROVIDERS).eq("kind", "video"),
    supabase.from("media").select("id, storage_path, thumbnail_path, caption").eq("daily_report_id", rdoId).in("external_provider", MEDIA_SOURCE_PROVIDERS).eq("kind", "file"),
